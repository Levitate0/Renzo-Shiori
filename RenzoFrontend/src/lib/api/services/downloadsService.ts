...
export const downloadsService = {
   ...
   */
  /** Each user's queue is isolated to their own library; viewAll (Owner-level only) shows every user's. */
  async getDownloadsByStatusWithCount(status: QueueStatus, limit: number = 100, keyword?: string, viewAll = false): Promise<DownloadInfoList> {
    const params = new URLSearchParams();
    params.append('status', status.toString());
    params.append('limit', limit.toString());
   ...
   */
  async getDownloadsMetrics(viewAll = false): Promise<DownloadsMetrics> {
    return apiClient.get<DownloadsMetrics>(`/api/downloads/metrics${viewAll ? '?viewAll=true' : ''}`);
  },
