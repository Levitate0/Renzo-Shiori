...
export const useWaitingDownloadsWithCount = (
  ...
  return useQuery({
    queryKey: ['downloads', 'waiting-with-count', limit, keyword, viewAll],
    queryFn: () => downloadsService.getDownloadsByStatusWithCount(QueueStatus.WAITING, limit, keyword, viewAll),
    staleTime: 30 * 1000, // 30 seconds
    refetchInterval: 60 * 1000, // Refetch every minute for real-time updates
    ...options,
  });
...
export const useCompletedDownloadsWithCount = (
  ...
  return useQuery({
    queryKey: ['downloads', 'completed-with-count', limit, keyword, viewAll],
    queryFn: () => downloadsService.getDownloadsByStatusWithCount(QueueStatus.COMPLETED, limit, keyword, viewAll),
    staleTime: 5 * 60 * 1000, // 5 minutes (completed downloads change less frequently)
    refetchInterval: 2 * 60 * 1000, // Refetch every 2 minutes
    ...options,
  });
...
export const useFailedDownloadsWithCount = (
  ...
  return useQuery({
    queryKey: ['downloads', 'failed-with-count', limit, keyword, viewAll],
    queryFn: () => downloadsService.getDownloadsByStatusWithCount(QueueStatus.FAILED, limit, keyword, viewAll),
    staleTime: 2 * 60 * 1000, // 2 minutes
    refetchInterval: 60 * 1000, // Refetch every minute
    ...options,
  });
...
export const useDownloadsMetrics = (
  ...
  return useQuery({
    queryKey: ['downloads', 'metrics', viewAll],
    queryFn: () => downloadsService.getDownloadsMetrics(viewAll),
