 ...
 */
export const useLibrary = (viewAll = false) => {
  return useQuery<SeriesInfo[]>({
    queryKey: ['series', 'library', viewAll],
    queryFn: () => seriesService.getLibrary(viewAll),
...
/**
 * Hook to get the distinct tags/genres available in the cached "Latest" cloud
 * catalogue, each with its occurrence count. Backing data for the browse tag filter.
 */
export const useLatestGenres = () => {
...
export const useUpdatesFeed = (start: number, count: number, viewAll = false) => {
  return useQuery<UpdateFeedItem[]>({
    ...
    queryFn: () => seriesService.getUpdates(start, count, viewAll),
...
/**
 * Hook to queue a download of every not-yet-downloaded chapter for a series.
 * On success the chapters + detail queries are invalidated so missing/attribution
 * state refreshes as the download jobs complete.
 */
export const useDownloadAllChapters = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (seriesId: string) => seriesService.downloadAll(seriesId),
    onSuccess: (_, seriesId) => {
...
/**
 * Hook to delete downloaded chapter files for a series. Pass chapterNumbers to
 * delete a selection, or omit for all. Chapters + detail queries are invalidated
 * so downloaded/missing state refreshes immediately.
 */
export const useDeleteDownloads = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ seriesId, chapterNumbers }: { seriesId: string; chapterNumbers?: number[] }) =>
      seriesService.deleteDownloads(seriesId, chapterNumbers),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: ['series', 'chapters', variables.seriesId] });
      void queryClient.invalidateQueries({ queryKey: ['series', 'detail', variables.seriesId] });
    },
