  ...
  return (
    <CardContent className="space-y-6">
      ...
      <div className="space-y-2 border-t pt-4">
        <div className="flex items-center space-x-2">
          <Switch
            id="download-all-chapters"
            checked={localSettings.downloadAllChapters === true}
            onCheckedChange={(checked) =>
              setLocalSettings((prev) => ({ ...prev, downloadAllChapters: checked }))
            }
          />
          <Label htmlFor="download-all-chapters">Download all chapters</Label>
        </div>
        <p className="text-xs text-muted-foreground">
          Pull every chapter of every series, ignoring each series&apos; start-point
          cutoff — including gaps below the newest downloaded. The update scan then
          fills the whole library instead of only new chapters. This can queue a very
  ...
  return (
    <CardContent className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <Label htmlFor="max-requests-per-host">
            Max Requests Per Host (5–12)
          </Label>
          <Input
            id="max-requests-per-host"
            type="number"
            min="5"
            max="12"
            value={localSettings.maxRequestsPerHost ?? 5}
            onChange={(e) =>
              setLocalSettings((prev) => ({
                ...prev,
                maxRequestsPerHost: Math.min(
                  12,
          ...
          />
          <p className="text-muted-foreground mt-1 text-sm">
            The real ceiling on per-source download speed: every request to one
            site shares this budget, no matter how many chapters run at once. 5
            is the default. Raise it to drain a large backlog on a single source
            faster.
          </p>
          <p className="mt-1 text-sm font-medium text-amber-600 dark:text-amber-500">
            ⚠ Ban risk: higher values hammer a single host harder. Values near 12
            make rate-limiting or an IP ban materially more likely on strict
            sources — raise this gradually and back off if a source starts
            failing.
          </p>
        </div>
        <div>
  ...
  return (
    <CardContent className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
        ...
        <div>
          <Label htmlFor="library-scan-hours">Library Rolling Scan (3–12 h)</Label>
          <Input
            id="library-scan-hours"
            type="number"
            min={3}
            max={12}
            step={1}
            value={localSettings.libraryScanIntervalHours ?? 6}
            onChange={(e) => {
              const v = Math.min(12, Math.max(3, parseInt(e.target.value) || 6));
              setLocalSettings((prev) => ({ ...prev, libraryScanIntervalHours: v }));
            }}
          />
          <p className="text-muted-foreground mt-1 text-sm">
            Full library check for new chapters on every source, every N hours.
            &quot;Update now&quot; on the Updates page runs the same scan immediately.
          </p>
        </div>
...
const getAllowedOriginsError = (raw: string): string | null => {
  ...
  return invalid
    ? `"${invalid}" is not a valid origin (e.g. https://renzo.example.com)`
    : null;
};
