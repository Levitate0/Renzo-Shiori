...
export function ConfirmSeriesStep({
  ...
  // Status authority is single-select — checking one source clears it from the
  // others (mirrors the "Use as Status" toggle on the series detail page).
        ...
        const updatedSeries: FullSeries[] = prev.fullSeries.map(
          (series: FullSeries): FullSeries => {
            ...
            return {
              ...series,
              useStatus:
                currentKey === seriesKey ? checked : checked ? false : series.useStatus,
            };
          },
        );
        return { ...prev, fullSeries: updatedSeries };
                  ...
                  {series.isSelected && !series.isUnselectable && (
                    <div className="src-row-extras">
                      ...
                      <div className="flex items-center gap-1.5">
                        <Switch
                          id={`status-${seriesKey}`}
                          checked={series.useStatus}
                          onCheckedChange={(checked) =>
                            handleStatusChange(seriesKey, checked)
                          }
                        />
                        <Label
                          htmlFor={`status-${seriesKey}`}
                          className="text-xs cursor-pointer"
                        >
                          Use as Status
                        </Label>
