...
export function SourcesList({
  ...
  // "Installed" here means installed FOR ME (isEnabledForMe) — a source
  // someone else installed but I haven't enabled still shows under Available,
  // with an "Install" button that just enables it for me (no separate toggle).
  const filteredInstalledExtensions = useMemo(() => {
    const list = extensions.filter((ext) => ext.isEnabledForMe);
  ...
  const filteredAvailableExtensions = useMemo(() => {
    let list = extensions.filter((ext) => !ext.isEnabledForMe);
  ...
  // ── Install handler ──────────────────────────────────────────────────────────
  // One button, two outcomes depending on system state — never a separate
  // "enable" step: if the source isn't installed anywhere yet, this is a real
  // install (downloads/compiles the extension, and it's a fresh "first
  // installer" for the whole instance). If someone else already installed it,
  // this just adds it to MY own enabled set — cheap, no re-fetch.
  const handleInstall = async (pkgName: string) => {
    const target = extensions.find((ext) => ext.package === pkgName);
    const alreadyInstalledSystemWide = target?.isInstaled === true;
    try {
      setActionLoading(pkgName);
      if (alreadyInstalledSystemWide) {
        await providerService.enablePackageForMe(pkgName);
      } else {
      ...
      setExtensions((prev) =>
        prev.map((ext) =>
          ext.package === pkgName ? { ...ext, isInstaled: true, isEnabledForMe: true } : ext
        )
      );
      doClearSearch?.();

      // Open preferences for the extension the user just enabled/installed
      if (target) {
        setShowPreferencesFor({ pkgName: target.package, name: target.name });
      }
    } catch (error) {
  ...
  // ── Uninstall handler ────────────────────────────────────────────────────────
  // Removes the source from MY own enabled set only — it stays installed
  // system-wide for anyone else who has it enabled.
  const handleUninstall = async (pkgName: string) => {
    try {
      setActionLoading(pkgName);
      await providerService.disablePackageForMe(pkgName);

      // Optimistic update
      setExtensions((prev) =>
        prev.map((ext) =>
          ext.package === pkgName ? { ...ext, isEnabledForMe: false } : ext
        )
      );
      doClearSearch?.();
    } catch (error) {
