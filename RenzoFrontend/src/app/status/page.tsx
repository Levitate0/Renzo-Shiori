...
export default function StatusPage() {
  ...
  const { canAdmin, canOwner } = useAuth();
  ...
  const effectiveViewAll = canOwner && viewAllLibraries;
  const { data: series, isLoading: seriesLoading } = useSeriesStatus(effectiveViewAll);
  const { data: providers, isLoading: providersLoading } = useProviderStatus(effectiveViewAll);
        ...
        <div className="flex w-full items-center gap-2">
          ...
          {canOwner && (
            <button
              ...
              title={viewAllLibraries ? "Showing every user's alerts — click to view only your own" : "Showing only your alerts — click to view every user's"}
              className={`ml-auto inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-medium transition-colors ${
                viewAllLibraries
