...
function SeriesPageContent() {
  ...
  const updateSeriesWithSwitches = async (providerId: string, newSwitches: { useTitle?: boolean; useCover?: boolean; useStorage?: boolean; useStatus?: boolean }) => {
    if (!series || isDeleting) return;
    ...
    try {
      ...
      const updatedSeries = {
        ...
        providers: series.providers.map(provider => {
          ...
          const currentSwitches = provider.id === providerId ? 
            ...
            (providerSwitches[provider.id] || { useTitle: false, useCover: false, useStorage: false, useStatus: false });
          const fromChapterValue = providerFromChapters[provider.id];
          ...
          const updatedProvider = {
            ...
            useStatus: currentSwitches?.useStatus ?? provider.useStatus,
          };
      ...
      if (result && result.providers) {
        const updatedFromChapters: Record<string, string> = {};
        const updatedDisabledStates: Record<string, boolean> = {};
        const updatedSwitches: Record<string, { useTitle: boolean; useCover: boolean; useStorage: boolean; useStatus: boolean }> = {};
        ...
        result.providers.forEach(provider => {
          ...
          updatedSwitches[provider.id] = {
            ...
            useStatus: provider.useStatus ?? false
          };
        });
  ...
  // Handler for use-status changes — single-select, mirrors cover/title. The
  // chosen provider becomes the authority for the series' release status.
  const handleUseStatusChange = async (providerId: string, enabled: boolean) => {
    const allUpdates: Record<string, { useStatus?: boolean }> = {};
    if (enabled) {
      Object.keys(providerSwitches).forEach(id => {
        if (id !== providerId) allUpdates[id] = { useStatus: false };
      });
    }
    allUpdates[providerId] = { useStatus: enabled };

    setProviderSwitches(prev => {
      const newSwitches = { ...prev };
      Object.keys(allUpdates).forEach(id => {
        if (newSwitches[id]) newSwitches[id] = { ...newSwitches[id], ...allUpdates[id] };
      });
      return newSwitches;
    });
  ...
  const handleSetCategory = async (category: string | null) => {
    if (!seriesId) return;
    try {
      const res = await seriesService.setCategory(seriesId, category);
      toast({
        title: res.moved ? "Series moved" : "Category updated",
        description: category
          ? `Now filed under ${category}.`
          : "Moved back to the library root.",
      });
  ...
  const updateSeriesWithCompleteState = async (overrideSwitches?: Record<string, { useTitle?: boolean; useCover?: boolean; useStorage?: boolean; useStatus?: boolean }>) => {
    if (!series) return;
    ...
    try {
      ...
      if (result && result.providers) {
        ...
        const newSwitches: Record<string, { useTitle: boolean; useCover: boolean; useStorage: boolean; useStatus: boolean }> = {};
  ...
  return (<>
    ...
    {/* New layout: full-width hero, then 12-col grid. On desktop the whole
        thing is a viewport-height flex column so the page itself never scrolls —
        only the chapter list scrolls internally (fits any window: browser/apk/exe). */}
    <div className="w-full lg:flex lg:h-full lg:min-h-0 lg:flex-col lg:overflow-hidden">
      ...
      {/* Main content — two-column: sources + downloads on the left, the
          always-visible chapters list on the right (stacks on mobile, with
          chapters first so the primary content leads). On desktop this region
          fills the remaining viewport height below the hero. */}
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 py-8 sm:py-10 lg:min-h-0 lg:flex-1 lg:py-6">
        <div className="grid grid-cols-1 gap-6 lg:h-full lg:min-h-0 lg:auto-rows-fr lg:grid-cols-[minmax(320px,380px)_1fr] lg:gap-8">

          {/* Left column — source management + recent downloads (scrolls
              internally on desktop if taller than the viewport) */}
          <div className="order-2 min-w-0 space-y-6 lg:order-1 lg:min-h-0 lg:overflow-y-auto">
            <SourcesSection
              series={series}
              ...
              onUseStatusChange={handleUseStatusChange}
          ...
          {/* Right column — the chapters list (fills column height on desktop) */}
