...
using RensaioBackend.Services.Reader;
...
public class ReaderController : ControllerBase
{
    ...
    public async Task<ActionResult> SetProgressAsync([FromBody] ReaderProgressRequestDto req, CancellationToken token = default)
    {
        ...
        if (series == null)
            return NotFound();
        // Progress only ever advances from the reader; explicit un-read goes through /mark.
        _readState.SetReadState(CurrentUsername, user?.Id ?? Guid.Empty, series.Id, req.Filename ?? "",
            null, "Rensaiō Reader", series.StoragePath, req.ChapterNumber, req.LastReadPage, req.TotalPages, updateLower: false);
    ...
    /// from inside the extension, not from Rensaio. Tell the user that plainly
