...
public class ImportCommandService
{
    ...
    private async Task ReconcileLanguagesFromImportAsync(List<RensaioBackend.Models.Database.ImportEntity> imports)
    {
    ...
    private async Task SaveImportsAsync(HashSet<string> existingFolders, List<ImportSeriesSnapshot> newSeries, bool titleOnly, CancellationToken token = default)
    {
        ...
        foreach (ImportSeriesSnapshot k in newSeries)
        {
            ...
            RensaioBackend.Models.Database.ImportEntity? import = imports.FirstOrDefault(a => a.Path.Equals(k.Path, StringComparison.InvariantCultureIgnoreCase));
            if (import != null)
            {
                bool change = false;
            ...
            {
                RensaioBackend.Models.Database.ImportEntity imp = new RensaioBackend.Models.Database.ImportEntity
                {
                    Title = k.Title,
                    Path = k.Path,
    ...
    {
        RensaioBackend.Models.Database.ImportEntity? import = await _db.Imports.FirstOrDefaultAsync(a => a.Path == info.Path, token).ConfigureAwait(false);
        if (import == null)
            return;
        ...
        try
        {
            foreach (RensaioBackend.Models.Database.ImportEntity import in imports)
            {
                if (import.Series != null && import.Series.Count > 0 && import.Action == Action.Add)
                {
                    ...
                    // AddSeriesAsync already calls _stateService.SyncToRensaioJsonAsync() internally,
                    // so no separate rensaio.json sync is needed here.
