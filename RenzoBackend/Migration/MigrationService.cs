...
using LegacyProvider = RenzoBackend.Migration.Models.ProviderStorage;
using LegacySuwayomiPreference = RenzoBackend.Migration.Models.SuwayomiPreference;
using LegacySuwayomiSource = RenzoBackend.Migration.Models.SuwayomiSource;
using NewProviderStorage = RenzoBackend.Models.Database.ProviderStorageEntity;
using SuwayomiSource = RenzoBackend.Migration.Models.SuwayomiSource;

namespace RenzoBackend.Migration;

/// <summary>
/// Coordinates the one-time migration from the legacy <c>kaizoku.db</c> file to the new <c>renzo.db</c> schema.
/// </summary>
public class MigrationService
{
    ...
    private async Task MigrateSettingsAsync(OldDbContext legacyDb, AppDbContext targetDb, MigrationState state, CancellationToken cancellationToken)
    {
        ...
        {
            targetDb.Settings.Add(new RenzoBackend.Models.Database.SettingEntity
            {
                Name = setting.Name,
                Value = setting.Value
            });
    ...
    private async Task MigrateSeriesProvidersAsync(OldDbContext legacyDb, AppDbContext targetDb, MigrationState state, CancellationToken cancellationToken)
    {
        ...
        var migratedProviders = new List<RenzoBackend.Models.Database.SeriesProviderEntity>(legacySeriesProviders.Count);
    ...
    private async Task MigrateSeriesAsync(OldDbContext legacyDb, AppDbContext targetDb, MigrationState state, CancellationToken cancellationToken)
    {
        ...
        var migratedSeries = new List<RenzoBackend.Models.Database.SeriesEntity>(legacySeries.Count);

        foreach (var series in legacySeries)
        {
            var destination = new RenzoBackend.Models.Database.SeriesEntity
            {
                Id = series.Id,
    ...
    private async Task MigrateJobsAsync(OldDbContext legacyDb, AppDbContext targetDb, MigrationState state, CancellationToken cancellationToken)
    {
        ...
        List<RenzoBackend.Migration.Models.Job> jobs = await legacyDb.Jobs
        .AsNoTracking()
        .ToListAsync(cancellationToken)
        .ConfigureAwait(false);
        ...
        List<RenzoBackend.Models.Database.JobEntity> migratedJobs = new List<RenzoBackend.Models.Database.JobEntity>(jobs.Count);
        foreach (var job in jobs)
        {
            RenzoBackend.Models.Database.JobEntity newJob = new RenzoBackend.Models.Database.JobEntity
            {
                Id = job.Id,
                JobType = job.JobType,
    ...
    private async Task<RenzoBackend.Models.Database.SeriesProviderEntity> ConvertSeriesProviderAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var destination = new RenzoBackend.Models.Database.SeriesProviderEntity
        {
            ...
            Chapters = legacyProvider.Chapters?.Select(MapChapter).ToList() ?? new List<RenzoBackend.Models.Chapter>()
        };
        ...
        string? language)
    {
        ...
        */
    private static RenzoBackend.Models.Chapter MapChapter(Models.Chapter chapter)
    {
        return new RenzoBackend.Models.Chapter
        {
            Name = chapter.Name,
    ...
    private static string ResolveSeriesThumbnail(Migration.Models.Series series, MigrationState state)
    {
        ...
        {
            RenzoBackend.Models.Database.SeriesProviderEntity? prov = state.SeriesProviders.FirstOrDefault(p => p.SeriesId == series.Id && !string.IsNullOrWhiteSpace(p.ThumbnailUrl));
            if (prov != null)
            {
                if (!prov.IsCover)
                {
                    RenzoBackend.Models.Database.SeriesProviderEntity? prev = state.SeriesProviders.FirstOrDefault(p => p.SeriesId == series.Id && prov.IsCover);
                    if (prev != null)
                    {
                        prev.IsCover = false;
                        prov.IsCover = true;
    ...
    private sealed class MigrationState
    {
        ...
        public List<RenzoBackend.Models.Database.SeriesProviderEntity> SeriesProviders { get; set; } = [];
