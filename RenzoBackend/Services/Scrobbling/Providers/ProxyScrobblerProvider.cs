...
namespace RenzoBackend.Services.Scrobbling.Providers;
...
public abstract class ProxyScrobblerProvider : IScrobblerProvider
{
    ...
    // Cached across all provider instances so the key file is read/generated once.
    private static string? _resolvedInstanceKey;
    private static readonly object _instanceKeyLock = new();

    /// <summary>
    /// The identifier this instance presents to the scrobbler OAuth proxy. It only
    /// needs to be non-empty and STABLE (the proxy rejects an empty key and isolates
    /// tokens per key) — it is NOT pre-registered anywhere. So rather than making
    /// every deployer configure one, we honor an explicit Scrobbling:InstanceKey /
    /// Scrobbling__InstanceKey override, else read a persisted key from the config
    /// dir, else generate one and save it. Zero setup for a fresh container.
    /// </summary>
    private static string ResolveInstanceKey(IConfiguration configuration, ILogger logger)
    {
        string? configured = configuration.GetValue<string>("Scrobbling:Proxy:InstanceKey")
                             ?? configuration.GetValue<string>("Scrobbling:InstanceKey");
        if (!string.IsNullOrWhiteSpace(configured))
            return configured.Trim();

        lock (_instanceKeyLock)
        {
            if (!string.IsNullOrEmpty(_resolvedInstanceKey))
                return _resolvedInstanceKey;

            string dir = configuration["runtimeDirectory"] ?? ".";
            string path = Path.Combine(dir, "scrobbler-instance.key");
            try
            {
                if (File.Exists(path))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(existing))
                        return _resolvedInstanceKey = existing;
                }
                string generated = Guid.NewGuid().ToString("N");
                File.WriteAllText(path, generated);
                logger.LogInformation("Scrobbler: generated a persistent instance key at {Path} (no manual config needed).", path);
                return _resolvedInstanceKey = generated;
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Scrobbler: couldn't persist an instance key; using a per-session one.");
                return _resolvedInstanceKey = Guid.NewGuid().ToString("N");
            }
        }
    }
        ...
        ScrobblerTokenProtector tokenProtector)
    {
        ...
        _proxyBaseUrl = configuration.GetValue<string>("Scrobbling:ProxyUrl")?.TrimEnd('/')
                        ?? "https://renzo-apps.levitatemedia.top/auth";
    ...
    public async Task<ScrobblerAuthUrlResult> GetAuthorizationUrlAsync(string redirectUri, string state)
    {
        ...
        // Tell the (now container-hosted) proxy the PUBLIC base URL so the OAuth
        // redirect_uri it registers matches the admin's provider app. Derived from the
        // caller's public request host (passed as redirectUri by ScrobblerController).
        _proxyHttpClient.DefaultRequestHeaders.Remove("X-Public-Base");
        if (Uri.TryCreate(redirectUri, UriKind.Absolute, out var pubUri))
            _proxyHttpClient.DefaultRequestHeaders.Add("X-Public-Base", $"{pubUri.Scheme}://{pubUri.Authority}");
