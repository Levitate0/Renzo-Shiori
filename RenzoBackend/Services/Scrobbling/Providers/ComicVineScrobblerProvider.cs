...
public class ComicVineScrobblerProvider : IScrobblerProvider
{
    ...
    public Task<Dictionary<decimal, float>> GetReadChaptersAsync(string externalSeriesId, CancellationToken token = default)
        => Task.FromResult(new Dictionary<decimal, float>());

    public Task<bool> SetReadChaptersAsync(string externalSeriesId, Dictionary<decimal, float> chapterState, CancellationToken token = default)
