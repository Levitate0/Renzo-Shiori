...
/// 2. ImportSeriesSnapshot.ExternalMappings (from renzo.json / DB snapshot)
...
public class SeriesMatchingService
{
    ...
    private readonly CategoryMaintenanceService _categoryMaintenance;
    ...
    public SeriesMatchingService(
        ...
        CategoryMaintenanceService categoryMaintenance,
    ...
    public async Task<AutoMatchResultDto> AutoMatchAllAsync(Guid userId, ScrobblerProvider provider, CancellationToken token = default)
    {
        ...
        // Now that fresh matches have stamped each series' ScrobblerType, re-file the
        // library on that authoritative signal only (MangaDex/heuristic moves excluded).
        // Never let a categorization hiccup fail the match run.
        try
        {
            await _categoryMaintenance
                .RecategorizeAllAsync(userId, dryRun: false, token, scrobblerSignalOnly: true)
                .ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Post-match auto-categorization failed for user {UserId}", userId);
        }
    ...
    /// Also upserts the global SeriesMapping and syncs renzo.json.
    /// </summary>
        ...
        string externalSeriesId, string? externalTitle = null, CancellationToken token = default)
    {
        ...
        // Best-effort: capture the scrobbler media-type for category resolution. There is
        // no details-by-id endpoint, so look the confirmed id back up via search and take
        // its reported type. Never fatal — falls back to MangaDex/heuristic if it misses.
        try
        {
            var series = await _db.Series.FirstOrDefaultAsync(s => s.Id == seriesId, token);
            if (series != null && !string.IsNullOrWhiteSpace(externalTitle))
            {
                var scrobbler = _providerFactory.GetProvider(provider);
                if (scrobbler != null)
                {
                    await scrobbler.EnsureAuthenticatedAsync(userId, token);
                    var results = await scrobbler.SearchSeriesAsync(externalTitle, token);
                    var matchResult = results.FirstOrDefault(r => r.ExternalId == externalSeriesId);
                    if (!string.IsNullOrWhiteSpace(matchResult?.Type))
                    {
                        series.ScrobblerType = matchResult!.Type;
                        await _db.SaveChangesAsync(token);
                    }
                }
            }
        ...
        {
            _logger.LogDebug(ex, "ConfirmMatch media-type lookup failed for series {SeriesId}", seriesId);
        }
        ...
        SeriesEntity? series, ScrobblerProvider provider, CancellationToken token)
    {
        ...
        // Step 2: Check renzo.json on disk for persisted ExternalMappings
        if (series != null && !string.IsNullOrEmpty(series.StoragePath))
        {
            try
            {
                ...
                var renzoJsonPath = System.IO.Path.Combine(seriesFolder, "renzo.json");

                if (File.Exists(renzoJsonPath))
                {
                    var jsonContent = await File.ReadAllTextAsync(renzoJsonPath, token).ConfigureAwait(false);
                    ...
                    if (snapshot?.Series.ExternalMappings!=null)
                    {
                        ...
                        if (mapping!=null)
                        {
                            _logger.LogDebug("Found renzo.json ExternalMappings cascade for series {SeriesId} provider {Provider}: {ExternalId} {ExternalTitle}",     series.Id, mapping.Provider, mapping.ExternalId, mapping.ExternalTitle);
            ...
            {
                _logger.LogWarning(ex, "Failed to read renzo.json for ExternalMappings cascade (series {SeriesId})", series.Id);
            }
        }
    ...
    /// After upserting, syncs renzo.json via SeriesStateService.
    /// </summary>
        ...
        CancellationToken token)
    {
        ...
        if (existing != null)
        {
            ...
            else
            {
                ...
                // Don't overwrite, but still sync renzo.json to ensure consistency
            }
        }
        else
        {
        ...
        // After upserting, sync renzo.json via SeriesStateService
        await _seriesStateService.SyncToRenzoJsonAsync(seriesId, token);
    }
        ...
        ScrobblerProvider provider, CancellationToken token)
    {
        ...
        // Capture the scrobbler's media-type (MAL manga/manhwa/manhua, Kitsu, …) as
        // the authoritative category signal. Only overwrite when the match actually
        // reports a type, so a typeless provider never wipes a good earlier value.
        if (!string.IsNullOrWhiteSpace(bestResult?.Type))
            series.ScrobblerType = bestResult!.Type;
