...
using RenzoBackend.Models.Opds;
...
namespace RenzoBackend.Services.Opds;

/// <summary>
/// Service for building OPDS (Open Publication Distribution System) Atom XML feeds.
/// Supports per-user read-state tracking via renzo.json.
/// </summary>
public class OpdsFeedService
{
    ...
    public async Task<(Stream, string mimeType)?> GetChapterStreamAsync(UserEntity user, Guid seriesId, string filename,CancellationToken token = default)
    {
        var series = await _db.Series.AsNoTracking().FirstOrDefaultAsync(a => a.Id == seriesId, token);
        if (series == null)
            return null;
        if (!Series.SeriesQueryService.CanAccessSeries(series.OwnerId, user.Id, user.Level == Models.Enums.UserLevel.Owner))
            return null;
    ...
    private StringBuilder RenderHeader(UserEntity user, string id, string title, string user_agent)
    {
        ...
        List<string> ids = new List<string> { "renzo", user.OpdsPath };
...
<feed xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns=""http://www.w3.org/2005/Atom"" xmlns:opds=""http://opds-spec.org/2010/catalog"">
  ...
  {thumb}");
    ...
    {
        var allSeries = await _db.Series.Include(a=>a.Sources).AsNoTracking()
            .Where(s => s.OwnerId == user.Id || s.OwnerId == Guid.Empty).ToListAsync(token);
    ...
    public async Task<string> BuildReadingFeedAsync(UserEntity user, string user_agent, CancellationToken token = default)
    {
        ...
        BuildHeader(sb, $"renzo:{EscapeXml(user.OpdsPath)}:reading", $"Reading - {EscapeXml(user.Username)}", user_agent);
    ...
    */
    ...
    public async Task<string> BuildSeriesFeedAsync(UserEntity user, string id, string title, string user_agent, string? category = null, string? tag = null, string? source = null, bool filterReading = false, bool last = false, CancellationToken token = default)
    {
        var seriesList = await _db.Series
            .Include(s => s.Sources)
            .AsNoTracking()
            .Where(s => s.OwnerId == user.Id || s.OwnerId == Guid.Empty)
            .ToListAsync(token);
    ...
    <id>renzo:{EscapeXml(user.OpdsPath)}:{EscapeXml(chapterId)}</id>
  {thumbnailLinks}
    ...
    */
    ...
    public async Task<string> BuildSeriesFeedAsync(UserEntity user, string user_agent, Guid seriesId, string? language = null, CancellationToken token = default)
    {
        ...
        if (series == null || !Series.SeriesQueryService.CanAccessSeries(series.OwnerId, user.Id, user.Level == Models.Enums.UserLevel.Owner))
    ...
    public async Task<string> BuildCategoriesFeedAsync(UserEntity user, string user_agent, CancellationToken token = default)
    {
        ...
        var allSeries = await _db.Series.AsNoTracking().Where(s => s.OwnerId == user.Id || s.OwnerId == Guid.Empty).ToListAsync(token);
    ...
    public async Task<string> BuildSourcesFeedAsync(UserEntity user, string user_agent, CancellationToken token = default)
    {
        ...
        var allSeries = await _db.Series.Include(a=>a.Sources).AsNoTracking().Where(s => s.OwnerId == user.Id || s.OwnerId == Guid.Empty).ToListAsync(token);
