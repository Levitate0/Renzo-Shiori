...
namespace RenzoBackend.Services.Helpers
{
    public class ArchiveHelperService
    {
        ...
        public async Task UpdateTitleAndAddComicInfoAsync(Guid seriesId, bool onlyDownloadByRenzo = true,
         CancellationToken token = default)
        {
            ...
            if (series == null)
                return;
            await UpdateTitleAndAddComicInfoAsync(series, onlyDownloadByRenzo, token).ConfigureAwait(false);
            await _db.SaveChangesAsync(token).ConfigureAwait(false);
        }
        public async Task UpdateTitleAndAddComicInfoAsync(Models.Database.SeriesEntity series, bool onlyDownloadByRenzo = true, CancellationToken token = default)
        {
            ...
            foreach (var sp2 in series.Sources)
            {
                ...
                foreach (var chap in sp.Chapters)
                {
                    ...
                    if (comicInfoName != null)
                    {
                        ...
                        {
                            if (oldComicInfo.Notes != null && oldComicInfo.Notes.Contains("Renzo"))
                            {
            ...
            // Sync renzo.json after archive rename or ComicInfo.xml update
        ...
        /// </summary>
        public async Task UpdateAllTitlesAndAddComicInfoAsync(ProgressReporter reporter, bool onlyDownloadByRenzo = true, CancellationToken token = default)
        {
            ...
            foreach (var series in allSeries)
            {
                ...
                await UpdateTitleAndAddComicInfoAsync(series, onlyDownloadByRenzo, token);
                acum += step;
            }
