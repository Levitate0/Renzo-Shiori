...
namespace RenzoBackend.Services.Providers
{
    ...
    public class ProviderManagerService
    {
        ...
        public async Task<List<ExtensionDto>> GetProvidersAsync(Guid userId, CancellationToken token = default)
        {
            try
            {
                //Local pass
                ...
                // Package -> whether the REQUESTING user has enabled at least one of
                // that package's MihonProviderIds for their own use.
                HashSet<string> enabledPackages = [];
                if (userId != Guid.Empty)
                {
                    List<string> enabledProviderIds = await _db.UserProviders
                        .Where(p => p.UserId == userId)
                        .Select(p => p.MihonProviderId)
                        .ToListAsync(token).ConfigureAwait(false);
                    var enabledSet = new HashSet<string>(enabledProviderIds, StringComparer.Ordinal);
                    enabledPackages = storages
                        .Where(s => enabledSet.Contains(s.MihonProviderId))
                        .Select(s => s.SourcePackageName!)
                ...
                foreach (RepositoryGroup repo in localRepos)
                {
                    ...
                    if (existing != null)
                    {
                        ...
                        v.IsInstaled = true;
                        v.IsEnabledForMe = enabledPackages.Contains(v.Package);
