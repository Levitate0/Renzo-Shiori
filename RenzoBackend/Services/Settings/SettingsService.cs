    ...
    public class SettingsService
    {
        ...
        public async Task SetTimesSettingsAsync(EditableSettingsDto set, CancellationToken token = default)
        {
            using (var scope = _prov.CreateScope())
            {
                ...
                // Rolling library scan — clamp to the supported 3-12h window.
                TimeSpan scanEvery = TimeSpan.FromHours(Math.Clamp(set.LibraryScanIntervalHours, 3, 12));
                settings.JobTimes[JobType.LibraryScan] = scanEvery;
                await jobManagment.SetRecurringTimeAsync(JobType.LibraryScan, scanEvery, token).ConfigureAwait(false);
            }
        }
        ...
        public async Task SaveSettingsAsync(EditableSettingsDto set, bool force = false, CancellationToken token = default)
        {
            ...
            // Push the per-host download concurrency to the live OkHttp client so the
            // change takes effect without a restart. Clamped to 5–12 inside the bridge.
            using (var okhttpScope = _prov.CreateScope())
            {
                okhttpScope.ServiceProvider.GetRequiredService<IBridgeManager>()
                    .SetMaxRequestsPerHost(set.MaxRequestsPerHost);
            }
        }
        ...
        public SettingsDto GetFromEditableSettings(EditableSettingsDto ed)
        {
            SettingsDto set = new SettingsDto
            {
                ...
                MaxRequestsPerHost = ed.MaxRequestsPerHost,
