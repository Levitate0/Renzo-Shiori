...
public static class ImportExtensions
{
    ...
    public static ImportSeriesSnapshot? ToImportSeriesSnapshot(this List<NewDetectedChapter> chapters)
    {
        ...
        var renzoMatches = chapters.Where(c => c.IsRenzoMatch).ToList();
        var nonRenzoMatches = chapters.Where(c => !c.IsRenzoMatch).ToList();
        var providers = renzoMatches.GroupBy(a => (a.Provider, a.Language)).ToDictionary(a => a.Key, a => a.ToList());

        if (nonRenzoMatches.Count > 0)
        {
            providers.Add(("Unknown", "en"), nonRenzoMatches);
        }
