...
using RenzoBackend.Services.Import.KavitaParser;
...
using Parser = RenzoBackend.Services.Import.KavitaParser.Parser;
using SeriesInfo = RenzoBackend.Services.Import.KavitaParser.SeriesInfo;
...
namespace RenzoBackend.Services.Import
{
    public class SeriesScanner
    {
        ...
        private static readonly Regex RenzoRegex = new Regex("^\\[(?<provider>[^\\]]+)\\](?:\\[(?<lang>[^\\]]+)\\])?\\s+(?<title>.+?)(?:\\s+(?<chapterNumber>-?\\d+(?:\\.\\d+)?))?\\s*(?:\\((?<chapterName>[^)]+)\\))?$",
        ...
        /// series/chapter/001.jpg — rather than Renzo's one-CBZ-per-chapter layout
        ...
        public async Task<ImportSeriesSnapshot?> ProcessDirectoryAsync(List<TachiyomiRepository> repos, string directoryPath, string seriesFolder, bool titleOnly = false, CancellationToken token = default)
        {
            ...
            foreach (LibraryType lib in libraryTypes)
            {
                ...
                foreach (var archiveFile in archiveFiles)
                {
                    ...
                    Match kavmatch = RenzoRegex.Match(kavname);
                    if (kavmatch.Success)
                    {
                        string language = "en";
        ...
        /// </param>
        public async Task RecurseDirectoryAsync(List<RenzoBackend.Models.Database.SeriesEntity> allseries, List<TachiyomiRepository> repos,
            ...
            foreach (var n in seriesFolders)
            {
                ...
                if (det != null)
                {
                    ...
                    if (!titleOnly)
                    {
                        ...
                        List<RenzoBackend.Models.Database.SeriesEntity> findMatchingSeries = seriesComparer.FindMatchingSeries(allseries, det);

                        if (findMatchingSeries.Count > 0)
                        {
                            Dictionary<RenzoBackend.Models.Database.SeriesEntity, ArchiveCompare> matches = [];
                            foreach (RenzoBackend.Models.Database.SeriesEntity s in findMatchingSeries)
                            {
                                matches.Add(s, seriesComparer.CompareArchives(det, s));
                            }

                            ArchiveCompare bt = ArchiveCompare.Equal;
                            KeyValuePair<RenzoBackend.Models.Database.SeriesEntity, ArchiveCompare>? r = matches.FirstOrDefault(a => (a.Value & ArchiveCompare.Equal) == ArchiveCompare.Equal);
