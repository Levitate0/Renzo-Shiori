...
using RenzoBackend.Services.Import.Models;
...
using Chapter = RenzoBackend.Models.Chapter;
using ImportEntity = RenzoBackend.Models.Database.ImportEntity;
using DbSeriesEntity = RenzoBackend.Models.Database.SeriesEntity;
...
public static class SeriesExtensions
{
        ...
        SeriesProviderEntity? provider = null, CancellationToken token = default)
    {
        ...
        provider.IsStatus = fs.UseStatus;
        provider.IsLocal = fs.IsLocal;
        // A provider that already tracks chapters is ESTABLISHED: a chapter it
        // doesn't know yet is a genuine new find, Updates-worthy. When the
        // provider is empty (series just added / first sync) its whole catalogue
        // is "new" — stamping those as found-now would flood the Updates feed
        // with the entire back-catalogue, so they take the publish date instead.
        bool establishedProvider = provider.Chapters.Count > 0;
        ...
        foreach (Chapter c in fs.Chapters)
        {
            ...
            if (pc != null)
            {
                // Backfill the "found" time for chapters recorded before it was
                // tracked, using a STABLE historical date (never "now") so they
                // don't flood the Updates feed as freshly-found on the next scan.
                pc.DateFetched ??= pc.ProviderUploadDate ?? pc.DownloadDate;
            ...
            {
                c.DateFetched = establishedProvider
                    ? foundNow
                    : (RenzoBackend.Extensions.ModelExtensions.HasRealUploadDate(c.ProviderUploadDate) ? c.ProviderUploadDate : null);
    ...
    {
        List<SeriesProviderEntity> list = providers as List<SeriesProviderEntity> ?? providers.ToList();

        // The series status comes from a designated authority, in priority order:
        // the explicit status source, then the permanent (storage) source. Only if
        // neither has a known status do we fall back to "best known across any".
        SeriesProviderEntity? authority = list.FirstOrDefault(a => a.IsStatus && a.Status != SeriesStatus.UNKNOWN)
            ?? list.FirstOrDefault(a => a.IsStatus)
            ?? list.FirstOrDefault(a => a.IsStorage && a.Status != SeriesStatus.UNKNOWN);
        if (authority != null)
            return authority.Status;

        SeriesProviderEntity? p = list.Where(a => a.Status != SeriesStatus.UNKNOWN).OrderBy(a => a.Status).FirstOrDefault();
        p ??= list.FirstOrDefault();
        return p?.Status ?? SeriesStatus.UNKNOWN;
    }
    ...
    public static void FillSeriesFromProviderSeriesDetails(this DbSeriesEntity dbSeries, ProviderSeriesDetails consolidatedSeries, decimal? startFromChapter)
    {
        ...
        // Series status follows the designated status source (BestStatus already
        // resolved it on the consolidated view). Without this the status source
        // toggle would save but never change the displayed status. Only overwrite
        // with a known value so a transient "unknown" consolidation doesn't wipe it.
        if (consolidatedSeries.Status != SeriesStatus.UNKNOWN)
            dbSeries.Status = consolidatedSeries.Status;
