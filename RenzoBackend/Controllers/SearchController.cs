...
namespace RenzoBackend.Controllers
{
    ...
    public class SearchController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<SearchSourceDto>>> GetAvailableSearchSourcesAsync(CancellationToken token = default)
        {
            try
            {
                var sources = await _searchQueryService.GetAvailableSearchSourcesAsync(CurrentUserId, token).ConfigureAwait(false);
            ...
            CancellationToken token = default)
        {
            ...
            try
            {
                var results = await _searchQueryService.SearchSeriesAsync(keyword, languageList, searchSources, 0.1f, CurrentUserId, token).ConfigureAwait(false);
