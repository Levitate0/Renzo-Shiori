...
using RenzoBackend.Services.Scrobbling.Providers;
...
public class ScrobblerController : ControllerBase
{
    ...
    {
        _logger.LogInformation("Scrobbler authorize requested for '{Provider}'.", provider);
        ...
        try
        {
            ...
            _logger.LogInformation("Scrobbler authorize for '{Provider}': got auth URL (len {Len}), state {State}.",
                provider, result.AuthUrl?.Length ?? 0, result.State);
        ...
        {
            _logger.LogError(ex, "Scrobbler authorize for '{Provider}' failed building the auth URL (proxy unreachable / rejected?).", provider);
            return StatusCode(502, new { message = "Could not reach the OAuth service. Try again shortly." });
        }
    }
    ...
    public async Task<ActionResult> ProxyCallback(string provider, [FromQuery] string state)
    {
        ...
        {
            _logger.LogInformation("Scrobbler {Provider} callback for user {User}: proxy token retrieval not ready (state={State}): {Err}",
                provider, userId, state, tokenResult.ErrorMessage ?? "no access token");
        ...
        _logger.LogInformation("Scrobbler {Provider} connected for user {User} (token stored, expires {Exp}).",
            provider, userId, tokenResult.ExpiresAt);
