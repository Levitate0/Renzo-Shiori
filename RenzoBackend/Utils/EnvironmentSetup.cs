...
namespace RenzoBackend.Utils
{

    public static class EnvironmentSetup
    {
        public const string AppRenzo = "Renzo";
        ...
        public static ILogger Logger
        {
            get
            {
                if (_logger == null)
                {
                    _logger = LoggerInfrastructure.CreateAppLogger(AppRenzo,nameof(EnvironmentSetup)); ;
                }
                return _logger;
            }
        }
        ...
        public static async Task WriteToAppSettingsAsync(string? storageDirectory, CancellationToken token = default)
        {
            ...
            if (storageDirectory == null)
                storageDirectory = Environment.GetEnvironmentVariable("RENZO_STORAGEDIR");
            if (storageDirectory==null && IsDocker)
            {
                storageDirectory = "/series";
            }
            string? importDirectory = Environment.GetEnvironmentVariable("RENZO_IMPORTDIR");
            ...
            {
                string resourceName = nameof(RenzoBackend) + "." + AppSettings;
            ...
            if (destinationJson != null)
            {
                ...
                if (connectionStrings != null)
                {
                    ...
                    {
                        dir = dir.Substring(0, dir.Length - 10) + "Renzo";
                    ...
                    {
                        string newDbPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(destPath) ?? "", "renzo.db");
                        try
                        {
                            if (!File.Exists(newDbPath))
                                File.Move(destPath, newDbPath);
                            string walDestPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(destPath) ?? "", "renzo.db-wal");
                            ...
                            walDestPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(destPath) ?? "", "renzo.db-shm");
                    ...
                    string expectedRelativeDb = "Data Source=renzo.db";
                    string expectedAbsoluteDb = "Data Source=" + System.IO.Path.Combine(Path, "renzo.db");
        ...
        public static void ExtractWWWRoot()
        {
            ...
            Stream? sha256Stream = assembly.GetManifestResourceStream(nameof(RenzoBackend) + "."+wwwRootSHA256);
            ...
            Stream? wwwStream = assembly.GetManifestResourceStream(nameof(RenzoBackend) + "." + wwwRootZip);
            if (wwwStream == null)
            {
        ...
        private static string VerifyIfRenameIsNeeded(string dataDir)
        {
            ...
            string finalDirectory = System.IO.Path.Combine(dataDir, "Renzo");
        ...
        {
            // Check for the RENZO_DATADIR environment variable, primarily for Docker containers.
            var dataDir = Environment.GetEnvironmentVariable("RENZO_DATADIR");
