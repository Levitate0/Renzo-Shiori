# Hosting Guide: Renzo OAuth Proxy on Cloudflare

This guide walks through deploying the [`RenzoOAuthProxy.CF`](RenzoOAuthProxy.CF/) Cloudflare Worker from scratch.
...
```powershell
npx wrangler d1 create renzo-oauth-proxy
```

**Output:**
```
✅ Successfully created DB 'renzo-oauth-proxy' in region WEUR

[[d1_databases]]
binding = "DB"
database_name = "renzo-oauth-proxy"
database_id = "your-database-id-here"
```

Copy the `database_id` value and paste it into [`wrangler.toml`](RenzoOAuthProxy.CF/wrangler.toml):
...
```powershell
npx wrangler d1 migrations apply renzo-oauth-proxy
```

This runs [`migrations/0001_create_sessions.sql`](RenzoOAuthProxy.CF/migrations/0001_create_sessions.sql) which creates the `oauth_sessions` table:
...
Edit [`wrangler.toml`](RenzoOAuthProxy.CF/wrangler.toml) and fill in your client IDs:
...
If you have a domain (e.g., `auth.renzo.net`) pointed to Cloudflare:

1. Add the domain to your Cloudflare account (DNS tab)
2. Update [`wrangler.toml`](RenzoOAuthProxy.CF/wrangler.toml) with your zone ID:

```toml
routes = [
  { pattern = "auth.renzo.net/*", zone_id = "your-zone-id" }
]
```

To find your zone ID: Cloudflare Dashboard → your domain → Overview → Zone ID (right sidebar).

**No custom domain?** Skip this step. The worker will be available at `renzo-oauth-proxy.your-subdomain.workers.dev`.
...
Uploaded renzo-oauth-proxy (2.21 sec)
Published renzo-oauth-proxy (0.33 sec)
  https://oauth.renzo.net (or https://renzo-oauth-proxy.your-subdomain.workers.dev)
```
...
```powershell
# Health check
curl https://oauth.renzo.net/health
# → {"status":"ok","service":"renzo-oauth-proxy"}

# Test with missing instance key (should return 401)
curl -X POST https://oauth.renzo.net/api/oauth/anilist/url
# → {"error":"X-Instance-Key header required"}

# Test with unknown provider (should return 400)
curl -X POST https://oauth.renzo.net/api/oauth/unknown/url
# → {"error":"Unsupported provider: unknown"}
```

---

## Step 8: Configure Renzo Backend

In your Renzo instance's [`appsettings.json`](RenzoBackend/appsettings.json), point the [`ProxyScrobblerProvider`](RenzoBackend/Services/Scrobbling/Providers/ProxyScrobblerProvider.cs) to your new worker:

```json
"Scrobbling": {
    "ProxyUrl": "https://oauth.renzo.net",
    "Proxy": {
